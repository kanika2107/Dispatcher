...
////////////////////////////////////////////////////////////////////////////////
  ...
  /*  struct iovec iov8[1];
   ...
   /* struct iovec iov10[1];
    char msg_header10[sizeof(MessageHeader)];
    iov10[0].iov_base = msg_header10;
    iov10[0].iov_len  = sizeof(msg_header10);
    int iovcnt10 = sizeof(iov10)/sizeof(struct iovec);
    bytes_read = readv(fd,iov10,iovcnt10);
    MessageHeader* msgheader10 = reinterpret_cast<MessageHeader*>(msg_header10);
    char buffer10[msgheader10->size];
    iov10[0].iov_base = buffer10;
    iov10[0].iov_len = sizeof(buffer10);
    iovcnt10 = sizeof(iov10)/sizeof(struct iovec);
    bytes_read = readv(fd,iov10,iovcnt10);
    if(bytes_read<0) err(2,"readv failed");
    //Read the header
    if(msgheader10->type==MessageType::PING)
    {
      Ping* pingmessage = reinterpret_cast<Ping*>(buffer10);
      std::cout << pingmessage->cookie << std::endl;
    }*/
...
//////////////////////////////////////////////RECIEVE A PONG/////////////////////////////////////////////
  ...
  /*  struct iovec iov7[1];
    ...
    int iovcnt7 = sizeof(iov7)/sizeof(struct iovec);
    std::cout << fd << std::endl;
    bytes_read = readv(fd,iov7,iovcnt7);
    MessageHeader* msgheader7 = reinterpret_cast<MessageHeader*>(msg_header7);
    char buffer7[msgheader7->size];
    iov7[0].iov_base = buffer7;
    iov7[0].iov_len = sizeof(buffer7);
    iovcnt7 = sizeof(iov7)/sizeof(struct iovec);
    bytes_read = readv(fd,iov7,iovcnt7);
    std::cout << ":)" << std::endl;
    std::cout << bytes_read << std::endl;
    if(bytes_read<0) err(2,"readv failed");
    //Read the header
    std::cout << msgheader7->size << std::endl;
    if(msgheader7->type==MessageType::ORDER_NACK)
    {
    	Nack* nackmessage = reinterpret_cast<Nack*>(buffer7);
    	//std::cout << nackmessage->order_id << std::endl;
    }

     else if(msgheader7->type==MessageType::ORDER_ACK)
    {
    	Ack* ackmessage = reinterpret_cast<Ack*>(buffer7);
    	//std::cout << ackmessage->order_id << std::endl;
    }

    else if(msgheader7->type==MessageType::EXEC)
    {
